...
def gpu_worker(
    ...
    while True:
        try:
            ...
            model_name, dataset, activation = task_queue.get(timeout=1)
            task_count += 1
            ...
            print(f"📋 Task: {model_name} → {dataset} ({activation})")

            # Run the evaluation
            start_time = time.time()
            success, message = run_evaluation(model_name, dataset, activation, gpu_id)
            end_time = time.time()
