...
class ZeroShotInference:
    ...
    def predict_similarity(self, texts: List[str], candidate_labels: List[str]) -> List[Dict[str, any]]:
        """
        Predict similarity scores between texts and candidate labels.
        Uses cosine similarity between embeddings.
        """
        # Encode texts and labels
        text_embeddings = self.model.encode(texts, convert_to_tensor=True, show_progress_bar=False)
        label_embeddings = self.model.encode(candidate_labels, convert_to_tensor=True, show_progress_bar=False)
        ...
        for i, text in enumerate(texts):
            ...
            for j, label in enumerate(candidate_labels):
                predictions.append({
                    'label': label,
                    'score': float(text_similarities[j])
                })
    ...
    def predict_single(self, text: str, candidate_labels: List[str], top_k: int = None) -> Dict[str, any]:
    ...
    def classify_with_threshold(self, text: str, candidate_labels: List[str], 
    ...
    def batch_classify(self, texts: List[str], candidate_labels: List[str], 
                      threshold: float = 0.5) -> List[Dict[str, any]]:
        """Classify multiple texts efficiently."""
        return [self.classify_with_threshold(text, candidate_labels, threshold) 
                for text in texts]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Zero-shot classification inference using similarity")
    ...
    parser.add_argument("--top-k", type=int, default=10, help="Return top-k predictions")
    ...
    if args.text:
        if args.suggest:
            # Suggestion mode
            suggestions = inference.suggest_labels(args.text, top_k=args.top_k, threshold=0.3)
            print(f"\nText: {args.text}")
            ...
            result = inference.classify_with_threshold(args.text, args.labels, args.threshold)
            print(f"\nText: {result['text']}")
            ...
            try:
                ...
                if command.startswith('classify '):
                    ...
                    if len(parts) == 2:
                        ...
                        result = inference.classify_with_threshold(text.strip(), labels, args.threshold)
