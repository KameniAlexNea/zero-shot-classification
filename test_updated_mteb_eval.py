...
def test_updated_knn_evaluator():
    ...
    print(f"  Total labels: {len(set(evaluator.y_train + evaluator.y_test))} unique")
...
def test_shared_label_mapping():
    ...
    print(f"  Train label range: {min(knn_evaluator.y_train)} - {max(knn_evaluator.y_train)}")
    print(f"  Test label range: {min(knn_evaluator.y_test)} - {max(knn_evaluator.y_test)}")
