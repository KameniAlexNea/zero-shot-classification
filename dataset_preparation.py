...
class DatasetPreparator:
    ...
    def split_by_labels(
        ...
        num_test_labels = max(
            1, int(len(frequent_labels_list) * self.config.test_ratio)
        )
        ...
        logger.info(
            f"Test labels: {len(test_labels)} (completely unseen during training)"
        )
    ...
    def prepare_datasets(self):
        ...
        train_dataset.save_to_disk(
            os.path.join(self.config.output_dir, "train_contrastive")
        )
        test_dataset.save_to_disk(
            os.path.join(self.config.output_dir, "test_contrastive")
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Prepare datasets for zero-shot classification training"
    )
    parser.add_argument(
        "--batch-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Ratio of labels to reserve for testing",
    )
    parser.add_argument(
        "--min-label-freq",
        type=int,
        default=1,
        help="Minimum label frequency to include",
    )
    parser.add_argument(
